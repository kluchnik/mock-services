...
echo 'Проверяем наличие python2'
python2 -V

echo 'Проверяем наличие библиотеки icap-сервера'
pip show icapserver

echo 'Производим конфигурирование icap-сервера'
mkdir /home/icap_server
echo > /home/icap_server/run.py
chmod +x /home/icap_server/run.py

cat > /home/icap_server/run.py << EOF
#!/bin/python2
import time
import threading
...
echo > /lib/systemd/system/icap.service
echo > /var/log/icap.log

cat > /lib/systemd/system/icap.service <<EOF
[Unit]
Description = test icap server
[Service]
Type=simple
PIDFile=/run/icap_server.pid
ExecStart = /home/icap_server/run.py
StandardOutput=file:/var/log/icap.log
StandardOutput=file:/var/log/icap.log
[Install]
WantedBy = multi-user.target
EOF

echo 'Запускаем icap-сервер)'
systemctl stop icap.service
systemctl start icap.service
systemctl status icap.service

echo 'Проверяем icap-сервер'
netstat -lnp | grep :1344